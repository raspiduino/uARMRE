...
static void pxa255gpioPrvRecalcIntrs(Pxa255gpio* gpio){
	...
	pxa255icInt(gpio->ic, PXA255_I_GPIO_all, gpio->detStatus[1] || gpio->detStatus[2] || (gpio->detStatus[0] &~ 3));
	pxa255icInt(gpio->ic, PXA255_I_GPIO_1, (gpio->detStatus[0] & 2) != 0);
	pxa255icInt(gpio->ic, PXA255_I_GPIO_0, (gpio->detStatus[0] & 1) != 0);
}
...
UInt8 pxa255gpioGetState(Pxa255gpio* gpio, UInt8 gpioNum){
	...
	if(gpio->AFRs[bSet] & bV) return ((gpio->AFRs[bSet] & bV) >> (gpioNum & 0x0F)) - 1 + PXA255_GPIO_AFR1;
